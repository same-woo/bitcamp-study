...
public class MemberHandler {
  ...
  public static void inputMember() {
    ...
    m.remainingDays = inputdays();
    m.gender = inputGender((char)0);
    m.locker = inputlocker();
    m.no = userId++;
  ...
  public static void printMembers() {
    ...
    for (int i = 0; i < length; i++) {
      ...
      System.out.printf("%d, %s, %d, %d, %s,%s\n", 
        m.no, m.name, m.age, m.remainingDays, toGenderString(m.gender),tolockerString(m.locker));
    }
  }
  ...
  public static void updateMember() {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (m.no == Integer.parseInt(memberNo)) {
        ...
        System.out.printf("남은기간(%d)? ", m.remainingDays);
        m.remainingDays = inputdays();
