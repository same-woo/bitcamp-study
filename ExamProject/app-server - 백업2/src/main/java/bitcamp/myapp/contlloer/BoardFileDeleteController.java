  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (boardDao.deleteFile(fileNo) == 0) {
        ...
        response.sendRedirect("/WEB-INF/jsp/board/detail?category=" + category + "&no=" + board.getNo());
      }

    } catch (Exception e) {
      sqlSessionFactory.openSession(false).rollback();
      request.setAttribute("refresh", "2;url=/board/detail?category=" + request.getParameter("category") + "&no=" + request.getParameter("no"));
